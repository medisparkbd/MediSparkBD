...
export default function ViewAdminsPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        setAddMsg({ kind: "err", text: data?.error ?? "Failed to add admin." });
        return;
      }
      setAddMsg({ kind: "ok", text: "Admin added successfully." });
      setAddEmail("");
      setAddName("");
      if (data?.admins) {
        const all = data.admins;
        setStaff(all.filter((a) => (a.role ?? "admin").toLowerCase() === "admin"));
      } else {
        void load();
      }
    } catch {
  ...
  if (!gate.ready) return <AccessLoading label="Loading Admin Users…" />;
  if (!allowed) {
    return (
      <AccessMessage
        title="Admin Users — Administration access required"
        message="Your role does not include permission to view admin users."
          ...
          {staff.map((member) => (
            ...
            >
              {member.photoUrl ? (
                ...
                <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-purple-500 text-sm font-extrabold text-white shadow-md">
